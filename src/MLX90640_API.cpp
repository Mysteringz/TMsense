...
int MLX90640_GetFrameData(uint8_t slaveAddr, uint16_t *frameData)
{
    ...
    // Bounded: at the 2 Hz subpage rate a new subpage is ready within ~500 ms.
    // Unbounded, a sensor that stops reporting ready hangs the node for good.
    ...
    while(dataReady == 0)
    {
        ...
        if(dataReady == 0 && millis() - wait_start_ms > 2000)
        {
            return -9;
        }
    }       
